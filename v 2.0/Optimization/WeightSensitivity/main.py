...
def regional_closs(percentage_miss_strength):
    ...
    closs = compute_closs(sampled_building_info, building_loss, retrof_ds_median, sa_simulated, im_bin)

    # Compute penalty term
    penalty_term = penalty(sampled_building_info, percentage_miss_strength, w, pd.DataFrame(index=[1,2,3,4,5,6,7,8]), retrof_percent_range, 'single-building-cumulated-inverse')
...
def compute_closs(sampled_building_info, building_closs, building_dsm, Sa_map, IM_bin):
    ...
    site_fatality = buildings.merge(total_loss, how = 'left')
    ...
    regional_collapse_loss = np.multiply(collapse_prob,site_fatality.iloc[:,1]).sum().sum()
    return regional_collapse_loss
